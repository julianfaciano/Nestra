...
describe('temporary nesting profiler', () => {
  it.each(['squares', 'concave', 'dense'] as const)(
    ...
    (kind) => {
      ...
      // Captured from the original engine AND original collision module before instrumentation.
      expect(
        createHash('sha256').update(JSON.stringify(plain)).digest('hex'),
      ).toBe(baseline[kind]);
      ...
      expect({ ...measured, diagnostics: counts }).toEqual(plain);
      ...
      expect(c.segmentPairs).toBe(c.segmentAabbRejected + c.exactSegmentTests);
      ...
      for (const layout of measured.layouts) {
        for (const [i, piece] of layout.pieces.entries()) {
          expect(polygonFitsInsideCanvas(piece.polygon, input.canvas)).toBe(
            true,
          );
          expect(
            input.pieces.find((p) => p.id === piece.pieceId)!.allowedRotations,
